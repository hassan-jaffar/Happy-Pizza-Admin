...
function ItemManagementTable() {
  ...
  async function handleUpdate() {
    ...
    try {
      const result = await axios.post(
        `https://localhost:5000/api/admin/updateitemmanagement/${itemid}/${categoryid}`,
        detail
      ).data;
      console.log(result);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const data = await (
          await axios.get(`https://localhost:5000/api/admin/getitemmanagement/${itemid}/${categoryid}`)
        ).data;
        setitem(data.data);
        console.log(item);
      } catch (error) {
